...
const BlogItem = ({ blog }) => {
  ...
  return (
    <StyledContainer>
      <PublishedDate dangerouslySetInnerHTML={{ __html: publishedDate }} />
      <PostInfo>
        <ImageMeta>
          ...
          <div className="published-date-mobile">{frontmatter.published}</div>
          <p>
            Posted by <Link to={`/author/${frontmatter.author.toLowerCase()}`}>{frontmatter.author}</Link> in
          </p>
          {categoryList}
        </ImageMeta>
